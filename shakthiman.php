...
if($_SESSION["role"]!=10)
{
  ...
  if($work==2)//add/edit levels
	...
	if($page>1)
	{
		?><a class="link" href="shakthiman.php?work=1&page=<?php echo $page-1?>"><?php echo "previous " ?></a> <?php
	}
	$adjacents=2;
	...
	for ($i=$start;$i<=$end;$i++)
	{
		...
		?><a class="link" href="shakthiman.php?work=1&page=<?php echo $i?>"><?php echo $i." " ?></a> <?php
	}		
	if($pagecount>$page)
	{
			?><a class="link" href="shakthiman.php?work=1&page=<?php echo $page+1?>"><?php echo "Next " ?></a> <?php
	}
	...
	else if($work==4) // view levels
	{
		...
		while($row = mysqli_fetch_assoc($ref))
		{
			echo "<a class =\"link\" href = \"shakthiman.php?work=4&l=".$row["name"]."\">".$row["name"]."</a>";
		}
